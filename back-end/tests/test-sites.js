...
describe('Sites API', function () {
  ...
  it('should include child category sites when filtering by parent category', async function () {
    ...
    expect(response.body.data.map((site) => site.id)).to.include(childCategorySite.id);

    const childResult = response.body.data.find((site) => site.id === childCategorySite.id);
  ...
  it('should support subcategory_id filtering', async function () {
    ...
    expect(response.body.success).to.equal(true);
    expect(response.body.data.map((site) => site.id)).to.include(childCategorySite.id);
    expect(response.body.data.map((site) => site.id)).to.not.include(ownedSite.id);
  });
  ...
  it('should exclude child category sites when include_children is false', async function () {
    ...
    expect(response.body.data.map((site) => site.id)).to.not.include(childCategorySite.id);
  });
  ...
  it('should support legacy subcategory text filtering', async function () {
    ...
    expect(response.body.success).to.equal(true);
    expect(response.body.data.map((site) => site.id)).to.include(legacySubcategorySite.id);
  });
  ...
  it('should return owned sites including pending review entries', async function () {
    ...
    expect(response.body.data.map((site) => site.id)).to.include(ownedSite.id);
    expect(response.body.data.map((site) => site.id)).to.include(pendingSite.id);
    expect(response.body.data.map((site) => site.id)).to.not.include(foreignSite.id);

    const pendingResult = response.body.data.find((site) => site.id === pendingSite.id);
