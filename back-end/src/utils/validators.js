...
export const updateProfileSchema = Joi.object({
  ...
  phone_number: Joi.string().pattern(/^[+]?[0-9]{8,15}$/).allow('').optional(),
...
export const siteCreateSchema = Joi.object({
  ...
  price_range: Joi.string().valid('BUDGET', 'MODERATE', 'EXPENSIVE', 'LUXURY').optional(),
  ...
  status: Joi.string().valid('DRAFT', 'PENDING_REVIEW', 'PUBLISHED', 'ARCHIVED', 'REPORTED').optional()
});
...
export const reviewSchema = Joi.object({
  ...
  visit_type: Joi.string().valid('BUSINESS', 'COUPLE', 'FAMILY', 'FRIENDS', 'SOLO').optional(),
...
export const reviewUpdateSchema = Joi.object({
  ...
  visit_type: Joi.string().valid('BUSINESS', 'COUPLE', 'FAMILY', 'FRIENDS', 'SOLO').allow(null).optional(),
...
export function validateRequest(schema, data, options = { abortEarly: false, stripUnknown: true }) {
